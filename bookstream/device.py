...
class iPhone(Device):
    ...
    def formatLink(self, link):
        ...
        >>> l = catalog.Link(url = 'http://www.archivestream.github.io/download/item.expub', type = 'application/expub+zip')
        >>> i = iPhone()
        >>> l = i.formatLink(l)
        >>> print l.get('url')
        expub://www.archivestream.github.io/download/item.expub
        """
        if 'application/expub+zip' == link.get('type'):
            newUrl = re.sub('^http', 'expub', link.get('url'))
            link.set('url', newUrl)
        return link
