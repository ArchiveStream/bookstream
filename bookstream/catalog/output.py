...
"""
Copyright(c)2008 Archive Stream. Software license AGPL version 3.
...
class CatalogToAtom(CatalogRenderer):
    ...
    fileExtMap = {
        'pdf'  : 'application/pdf',
        'expub' : 'application/expub+zip',
        'mobi' : 'application/x-mobipocket-ebook'
    }

    ebookTypes = ('application/pdf',
                  'application/expub+zip',
    ...
    def createOpdsLink(self, entry, link):
        ...
        if copies:
            ...
            sub.attrib['total'] = '1' # currently archivestream.github.io only makes one copy available at a time
        ...
        if availability:
            expub_ac = ET.SubElement(element, self.opdsNS+'indirectAcquisition')
            expub_ac.attrib['type'] = 'application/vnd.adobe.adept+xml'
            sub = ET.SubElement(expub_ac, self.opdsNS+'indirectAcquisition')
            sub.attrib['type'] = 'application/expub+zip'
            pdf_ac = copy.copy(expub_ac)
    ...
    def createOpdsEntry(self, opds, obj, links, fabricateContentElement):
        ...
        if 'identifier' in obj:
            ...
            #urn = 'urn:x-internet-archive:bookstream:' + nss
...
class CatalogToHtml(CatalogRenderer):
    ...
    entryLinkTitles = {
        'application/pdf': 'PDF',
        'application/expub': 'exPub',
        'application/expub+zip': 'exPub',
    ...
    def createSearch(self, opensearchObj, query = None):
        ...
        if url is None:
            ...
            form = ET.SubElement(div, 'form', {'class':'opds-search-form', 'action':'/bookstream/catalog/search', 'method':'get' } )
    ...
    def createEntry(self, entry):
        ...
        <p class="opds-entry">
          ...
          <div class="opds-entry-links">
            <span class="opds-entry-item"><em class="opds-entry-key">Buy:</em> <a href="http://archivestream.github.io/download/itemid.pdf" class="opds-entry-link">PDF</a></span>
          </div>
        </p>
        """
    ...
    def createEntryLinks(self, links):
        ...
        >>> expub = Link(url = 'http://a.o/item.expub', type='application/expub+zip', rel='http://opds-spec.org/acquisition')
        >>> links = [pdf, expub]
        >>> e = testToHtml.createEntryLinks(links)
        >>> print ET.tostring(e, pretty_print=True)
        <div class="opds-entry-links">
          <span class="opds-entry-item"><em class="opds-entry-key">Free:</em> <a href="http://a.o/item.pdf" class="opds-entry-link">PDF</a>, <a href="http://a.o/item.expub" class="opds-entry-link">exPub</a></span>
        </div>
        """
    ...
    def createEntryLink(self, link):
        ...
        >>> l = Link(url = '/blah.expub', type='application/expub')
        >>> e = testToHtml.createEntryLink(l)
        >>> print ET.tostring(e)
        <a href="/blah.expub" class="opds-entry-link">exPub</a>
        """
    ...
    def isEbook(self, entry):
        for link in entry.getLinks():            
            if 'application/pdf' == link.get('type'):
                return True
            elif 'application/expub+zip' == link.get('type'):
                return True
    ...
    def addEntry(self, entry):
        ...
        for link in entry.getLinks():            
            if 'application/pdf' == link.get('type'):
                ...
                self.addField(doc, 'format', 'expub')
                self.addField(doc, 'link', link.get('url'))
                if link.get('price'):
                    price = link.get('price')
                    currencyCode = link.get('currencycode')
...
def testmod():
    ...
    testCatalog = Catalog(title='Archive Stream OPDS', urn=urn)
    testLink    = Link(url  = 'http://archivestream.github.io/download/itemid.pdf',
