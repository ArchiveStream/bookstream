...
feeds = (
         {'domain':'IA', 'url':'http://archivestream.github.io/bookstream/catalog/crawlable', 'sorted_by_date':False},
    ...
    for e in f.entries:
        ...
        for l in e.links:
            if 'application/pdf' == l.type:
                ...
                gotBook = True
            elif 'application/expub+zip' == l.type:
                addField(doc, 'format', 'expub')
                addField(doc, 'link',   l.href)
                gotBook = True 
...
def crawlFeedOnePage(feed, queue, crawlDateTime, latestWarc, warcDateTime, latestDateTime, tempdir):
    ...
    data = httpc.get(url, headers = {"User-Agent": "Archive Stream OPDS Crawler +http://archivestream.github.io/bookstream",})
